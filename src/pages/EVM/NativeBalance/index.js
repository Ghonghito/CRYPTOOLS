  ...
  const checkNativeBalance = async () => {
    ...
    if (address === '' || address === null || address === undefined) {
      ...
      if (String(address).includes(',')) {
        ...
        const data = await getEVMNativeBalance(addressList)
        setBalances(data)
      } else {
        const data = await getEVMNativeBalance(Array(address))
        console.log(data)
        setBalances(data)
      }
    }
    ...
    <div>
      <div className='flex items-center justify-center'>
        <div className='w-full lg:w-[550px] xl:w-[600px]'>
          <Card title='EVM NATIVE BALANCE CHECKER'>
            <div className='p-2'>
              <div className='mb-2 bg-darkBackground p-1 rounded-md border-[1px] border-darkBorder'>
                <Typography color='text-gray-500'>Check one or multiple wallet addresses for Native balance at once.</Typography>
              ...
              <div className='flex flex-col md:flex-row md:items-center gap-2'>
                <Input onKeyDown={(e) => e.key === 'Enter' && checkNativeBalance()} id='evmWalletAddress' placeholder='wallet address or addresses and separate them with commas' />
                <Button onClick={() => checkNativeBalance()} loading={isLoading}>
                  CHECK
                </Button>
              </div>
