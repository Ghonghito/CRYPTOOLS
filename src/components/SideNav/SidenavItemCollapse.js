...
const SidenavItemCollapse = ({ icon, name, path, pathname, collapse }) => {
  ...
  return (
    <div className='py-1'>
      <div
        className={`${active ? 'bg-primary shadow-lg' : ''} group flex items-center justify-between space-x-2 cursor-pointer duration-150 hover:bg-darkHover w-full h-[50px] px-2 rounded-md`}
        onClick={() => (collapsed === name ? setIsCollapsed(!collapsed) : setIsCollapsed(!collapsed))}>
        <div className='flex items-center space-x-2'>
          {path === '/aptos' ? <img src={icon} alt={name} className='w-5' /> : <div className={`${active ? 'text-white' : 'text-darkText'} duration-150 group-hover:text-white text-xl`}>{icon}</div>}
          <h1 className={`${active ? 'text-white' : 'text-darkText'} group-hover:text-white duration-150 text-sm`}>{name}</h1>
        </div>
        {collapsed ? (
          <IoIosArrowUp className={`duration-150 text-white group-hover:text-white ${active ? 'text-white' : 'text-primary'}`} />
        ) : (
          <IoIosArrowDown className={`duration-150 text-white group-hover:text-white ${active ? 'text-white' : ''}`} />
        )}
      </div>
      <Transition
        show={collapsed}
