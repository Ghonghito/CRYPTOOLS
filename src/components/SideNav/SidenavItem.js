...
const SidenavItem = ({ menuItem }) => {
  ...
  const renderRoutes = menuItem.map(({ type, path, name, icon, key, collapse }) => {
    ...
    if (type === 'noncollapsible') {
      returnValue = (
        <div key={key} className='mt-1'>
          <NavLink exact='true' to={path}>
            <div className={`${active ? 'bg-primary shadow-md' : 'hover:bg-primary'} group flex items-center space-x-2 cursor-pointer duration-150 hover:bg-darkHover w-full h-[50px] px-2 rounded-md`}>
              <div className='text-darkText duration-150 group-hover:text-white text-xl'>{icon}</div>
      ...
      returnValue = (
        <p key={Math.random(0, 99)} className='text-zinc-700 text-sm mt-3'>
          {name}
        </p>
      )
